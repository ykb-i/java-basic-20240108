...
// 캡슐화 : 인스턴스 필드와 메서드를 하나로 묶어서 필드에 대한 조작을 막는것
// 정보 은늑과 데이터 보호, 데이터 무결성을 보장할 수 있음

// Entity : 데이터베이스의 레코드를 관리하는 객체
// (생성자와 Getter만 유지) <= 권장사항
// DTO (Data Transfer Object): 데이터 전송을 위한 객체
// (불변성: 생성자와 Getter만 유지 / 가변성 : 생성자와 Getter, Setter 모두 유지)
// VO (Value Object) : 값을 표현하는 객체
// 가변성을 인정 X => 생성자와 Getter만 유지, Object 클래스의 equals 메서드를 오버라이딩해서 사용)
class Player{
	private String name;
	private String team;
